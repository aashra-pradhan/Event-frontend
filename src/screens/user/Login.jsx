...
function Login() {
  ...
  const onSubmit = async (formData) => {
    ...
    if (data1.status === 200 || data1.status === 201) {
      ...
      console.log(data1.data.data, "aashrutest");
      ...
      localStorage.setItem("userRole", data1.data.data.data.role);
      ...
      {
        data1.data.data.data.role === "admin"
          ? navigate("/admin/dashboard")
          : navigate("/");
      }
      //home bhanekai / ho, so home ma redirect gardincha ani navigate(0) le reload garaidincha so aba private home page dekhcha not public home page
      navigate(0);
    }
